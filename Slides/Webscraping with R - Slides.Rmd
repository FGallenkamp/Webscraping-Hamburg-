...
- Beispiele: [Twitter](https://developer.twitter.com/en.html), [Die Zeit](http://developer.zeit.de/index/) oder [Eurostat](https://ec.europa.eu/eurostat/web/json-and-unicode-web-services)
    
dynamische Webseiten & bot blocking:

- **erschwert** Zugriff, da Daten erst nach Nutzeranfragen erzeugt werden
- Süddeutsche Zeitung, ArtFacts
</div>

<div style="float: left; width: 40%;">
```{r echo = FALSE, eval = TRUE}
api.plot.df <- readRDS(file = "./Figures/api_plot_df.R")

api.plot <- ggplot(api.plot.df, aes(x = Submitted, y = cumulative.n)) +
